...
import { recognitionGalleryItems } from "@/data/recognition-gallery";

export default function RecognitionGalleryPage({ onNavigateRoute, onOpenBooking, phone }: { onNavigateRoute: (path: string) => void; onOpenBooking: () => void; phone: string; }) {
  return (
    <main className="min-h-screen bg-[#FBF3E7] text-[#241C1A]" style={{ fontFamily: "'Poppins', sans-serif" }}>
      <section className="relative overflow-hidden bg-[linear-gradient(135deg,_rgba(92,17,25,0.92),_rgba(58,11,16,0.92))] px-4 py-20 text-white sm:px-6 lg:px-8">
        ...
        <div className="relative mx-auto max-w-7xl">
          <p className="text-sm font-semibold uppercase tracking-[0.28em] text-[#F3D8A3]" style={{ fontFamily: "'Cinzel', serif" }}>Honours & Trophies</p>
          <h1 className="mt-4 text-4xl font-black tracking-tight text-white sm:text-5xl" style={{ fontFamily: "var(--font-dev), serif" }}>
            Honours & Trophies — Full Gallery
          </h1>
          <p className="mt-5 max-w-3xl text-base leading-8 text-white/75">
            Explore the complete collection of trophies, awards, and honours that highlight Guruvarya Shri Prakashbhau Shinde's journey of devotion, service, and community recognition.
          </p>
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <div className="grid gap-8 sm:grid-cols-2 xl:grid-cols-3">
          {recognitionGalleryItems.map((item) => (
            <div key={item.src} className="group overflow-hidden rounded-[1.85rem] border border-transparent bg-white p-3 shadow-[0_6px_20px_rgba(0,0,0,0.08)] transition-all duration-300 hover:border-[#C9A227]/30 hover:shadow-[0_12px_28px_rgba(0,0,0,0.14)]">
              <div className="overflow-hidden rounded-[1.5rem] bg-[#F9F3E8]">
                <img src={item.src} alt={item.title ?? "Honours & Trophies"} className="h-72 w-full object-cover transition-transform duration-500 group-hover:scale-105" loading="lazy" />
              </div>
              {item.title && (
